...
Usage (from project root):
  python scripts/training/run_evaluation.py
  python scripts/training/run_evaluation.py --data-dir dataset/samples --model-path server/models/best_model.pt
  python scripts/training/run_evaluation.py --ablations-only  # skip full eval, only ablation table
"""
...
def main():
    ...
    if not model_path.exists():
        print(f"Model not found: {model_path}. Train first (e.g. scripts/training/train_and_evaluate.py).")
        return 1
