...
Run from project root: python scripts/db/backfill_db_from_files.py
"""
...
def main():
    ...
    session.commit()
    session.close()
    print(f"Backfill done. Added {added} device(s). Run python scripts/db/check_db.py to verify.")
