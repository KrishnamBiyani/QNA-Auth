...
4. **Model optimization** - Achieve <1% FAR/FRR with minimal samples
   - **Optimization**: The current `approximate_entropy` calculation is O(N^2) and takes ~3s per sample. This slows down enrollment significantly. We should optimize this using Numba, Cython, or by reducing the sample size/parameters for production.
