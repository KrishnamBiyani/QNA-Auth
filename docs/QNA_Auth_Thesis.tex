...
\section{Measured Modality Context}

The repository also contains modality-specific evaluation JSON files under \texttt{model/evaluation/}. These files are useful because they show the broader measured range already available in the project, rather than only the strongest April 22 artifact slice. Table \ref{tab:measuredmodalitymetrics} summarizes the most thesis-relevant measured camera and microphone results that are already used in the report assets.

\begin{table}[H]
\centering
\caption{Measured modality-level results from repository evaluation artifacts}
\label{tab:measuredmodalitymetrics}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{p{0.22\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}}
\toprule
\textbf{Configuration} & \textbf{ROC-AUC} & \textbf{PR-AUC} & \textbf{EER} & \textbf{Accuracy} & \textbf{FAR} & \textbf{FRR} \\
\midrule
Camera only (measured) & 0.8579 & 0.8820 & 0.2678 & 0.7887 & 0.0622 & 0.3655 \\
Microphone only (measured) & 0.7096 & 0.7156 & 0.3641 & 0.6457 & 0.3050 & 0.4016 \\
April 22 capstone artifact (measured) & N/A & N/A & 0.1000 & 0.8333 & 0.2000 & 0.0000 \\
\bottomrule
\end{tabular}
\end{table}

The camera result is stronger and more stable than the microphone result under the saved modality reports. This supports the engineering decision already reflected in the runtime policy: camera should remain the primary source, while microphone should be treated as supportive evidence rather than as an equal or stronger signal.

\section{Projected Future Results}

Because the current dataset is small, the thesis should not imply that future performance is already known. At the same time, the repository now includes a dedicated forecast bundle under \texttt{Predicted report/} to support a viability discussion. These projected values are not experimental outcomes. They are scenario estimates based on the current measured artifact range, expected benefits of multi-session data collection, and a camera-dominant fusion policy with better threshold calibration.

Table \ref{tab:projectedfuturemetrics} summarizes three bounded forecast scenarios. The target scenario is the most defensible one to discuss in the thesis because it stays close to the strongest current artifact while still showing how the system could improve with more devices, more sessions, and better calibrated fusion.

\begin{table}[H]
\centering
\caption{Projected future fused-system metrics under larger multi-session data}
\label{tab:projectedfuturemetrics}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{p{0.22\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}p{0.1\textwidth}}
\toprule
\textbf{Scenario} & \textbf{ROC-AUC} & \textbf{PR-AUC} & \textbf{EER} & \textbf{Accuracy} & \textbf{FAR} & \textbf{FRR} \\
\midrule
Conservative fusion (projected) & 0.900 & 0.890 & 0.120 & 0.850 & 0.110 & 0.130 \\
Target fusion (projected) & 0.930 & 0.920 & 0.080 & 0.890 & 0.070 & 0.090 \\
Stretch fusion (projected) & 0.950 & 0.940 & 0.060 & 0.920 & 0.050 & 0.070 \\
\bottomrule
\end{tabular}
\end{table}

The value of this projection is not that it proves future performance. Its value is that it shows a technically reasonable path forward. Under the target scenario, the system would likely remain below the best offline PRNU forensic pipelines, but it would compare more favorably against short-sample live verification settings and older acoustic-only approaches.
...
Figures \ref{fig:projectedroc} and \ref{fig:projectedpr} show the corresponding projected ROC and PR curves from the forecast bundle. These figures are included only to visualize the future-work hypothesis under larger, better-calibrated data collection. They must be read as scenario illustrations, not as measured repository output.

\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{\detokenize{../Predicted report/images/projected_roc_curves.png}}
\caption{Projected ROC curves for conservative, target, and stretch future fusion scenarios}
\label{fig:projectedroc}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{\detokenize{../Predicted report/images/projected_pr_curves.png}}
\caption{Projected precision-recall curves for conservative, target, and stretch future fusion scenarios}
\label{fig:projectedpr}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{\detokenize{../Predicted report/images/measured_vs_projected_metrics.png}}
\caption{Measured camera and microphone results versus the projected fused target scenario}
\label{fig:measuredvsprojected}
\end{figure}
...
Fourth, the future-work projection is useful only because it is separated from measured evidence. The projected bundle suggests that more data, more sessions, and better source fusion could move the system toward approximately 0.93 ROC-AUC and 0.08 EER under the target scenario. That is a viability argument, not a completed result. Keeping that distinction explicit preserves the academic honesty of the thesis while still showing that the idea has room to mature.
...
These limitations do not invalidate the project. They define the boundary within which both the measured and projected results should be interpreted. In particular, the projected figures in \texttt{Predicted report/} are justified only as future-work estimates under explicit assumptions, not as experimentally verified claims.
